...
'''   
W_values=np.asarray(np.random.uniform(low=-1.0,high=1.0,size=(10,20)),dtype=theano.config.floatX)
bvis_values=np.zeros(shape=(10,),dtype=theano.config.floatX)
bhid_values=np.zeros(shape=(20,),dtype=theano.config.floatX)
...
#test
in_samples=np.random.rand(5,10)
print gibbs10(in_samples)

#6. 使用shared变量-the strict flag
'''
正如前面所说，传递shared变量给scan会得到更简单的计算图，从加快优化和计算的速度。
一个记住传递shared变量的方法是使用`strict`标志。当设置为true时，scan假设所有的
shared变量都作为non_sequences的一部分传给了fn函数，如果忘记传递就会导致一个错误。
'''
#using strict=True
values,updates=theano.scan(fn=OneStep,
                           outputs_info=sample,
...
#7. 多输出，多个taps值-Recurrent Neural Network with scan
'''
上面的例子展示了scan的简单用法。然而，scan不仅仅支持应用当前值和前一个值，而且可以向前多步。
...
def oneStep(u_tm4,u_t,x_tm3,x_tm1,y_tm1,W,W_in_1,W_in_2,W_feedback,W_out):
    ...
    return [x_t,y_t]
'''
正如变量的命名约定，我们使用a_tmb表示第t-b词的a,a_tpb表示在t+b的a。
注意参数的顺序和结果返回的顺序。尝试遵守taps（序列或输出的time slices）之间的时间顺序。
...
([x_vals,y_vals],updates)=theano.scan(fn=oneStep,
                                      ...
                                      non_sequences=[W,W_in_1,W_in_2,W_feedback,W_out],
                                        strict=True)
'''
`x_vals`和`y_vals`是符号变量，表示对`u`进行迭代产生的序列x和y。
`sequence_taps`和`outputs_taps`告诉scan需要哪些slices。s
'''                                        
                        
#8. 使用条件结束scan
'''
scan也可以作为`repeat-until`块使用。这样，scan满足条件或者达到最大迭代次数就会结束。
'''                        
